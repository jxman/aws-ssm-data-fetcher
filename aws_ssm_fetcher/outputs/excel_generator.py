...
class ExcelGenerator(BaseOutputGenerator):
    ...
    def __init__(self, context: OutputContext):
        ...
        self.green_fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")  # Light green for ✓
        self.red_fill = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")    # Light red for ✗
        self.header_fill = PatternFill(start_color="366092", end_color="366092", fill_type="solid") # Dark blue for headers
    ...
    def _generate_all_sheets(self, data: List[Dict]) -> Dict[str, pd.DataFrame]:
        ...
        regions = df['Region Code'].unique().tolist() if 'Region Code' in df.columns else []
    ...
    def _generate_service_matrix(self, data: List[Dict]) -> pd.DataFrame:
        ...
        services = sorted(df['Service Name'].unique()) if 'Service Name' in df.columns else []
        regions = sorted(df['Region Code'].unique()) if 'Region Code' in df.columns else []
        ...
        for service in services:
            row = {'Service': service}
            service_regions = set(df[df['Service Name'] == service]['Region Code'].tolist())
    ...
    def _generate_region_summary(self, data: List[Dict], regions: List[str]) -> pd.DataFrame:
        ...
        for region in regions:
            region_data = df[df['Region Code'] == region] if 'Region Code' in df.columns else df
            ...
            launch_date = "N/A"
            if (self.context.rss_data and 
                region in self.context.rss_data and 
                'launch_date' in self.context.rss_data[region]):
    ...
    def _generate_service_summary(self, data: List[Dict]) -> pd.DataFrame:
        ...
        total_regions = df['Region Code'].nunique() if 'Region Code' in df.columns else 1
        summary_data = []
        ...
        for service in services:
            ...
            region_count = service_data['Region Code'].nunique() if 'Region Code' in service_data.columns else 1
    ...
    def _generate_statistics(self, data: List[Dict]) -> pd.DataFrame:
        ...
        unique_regions = df['Region Code'].nunique() if 'Region Code' in df.columns else 0
        unique_services = df['Service Name'].nunique() if 'Service Name' in df.columns else 0
        
        avg_services_per_region = total_combinations / unique_regions if unique_regions > 0 else 0
        avg_regions_per_service = total_combinations / unique_services if unique_services > 0 else 0
        
        # Generate statistics data
        stats_data = [
            {'Metric': 'Total Service-Region Combinations', 'Value': total_combinations},
            {'Metric': 'Unique Regions', 'Value': unique_regions},
            {'Metric': 'Unique Services', 'Value': unique_services},
            {'Metric': 'Average Services per Region', 'Value': f"{avg_services_per_region:.2f}"},
            {'Metric': 'Average Regions per Service', 'Value': f"{avg_regions_per_service:.2f}"},
            {'Metric': 'Data Source', 'Value': 'AWS SSM Parameter Store'},
            {'Metric': 'Generated At', 'Value': self.context.metadata.get('generated_at', 'N/A')}
        ]
    ...
    def _apply_excel_formatting(self, writer: pd.ExcelWriter, sheets_data: Dict[str, pd.DataFrame]):
    ...
    def _adjust_column_widths(self, worksheet, df: pd.DataFrame):
        ...
        for column in worksheet.columns:
            ...
            # Get column name
            column_name = df.columns[column_index] if column_index < len(df.columns) else ''
    ...
    def _log_excel_details(self, sheets_data: Dict[str, pd.DataFrame], stats: Dict[str, int]):
        ...
        if 'Service Matrix' in sheets_data:
            ...
            self.logger.info(f"  - Service Matrix: {len(matrix_df)} services × {regions_count} regions")
        ...
        if 'Statistics' in sheets_data:
            self.logger.info(f"  - Statistics: {len(sheets_data['Statistics'])} metrics")
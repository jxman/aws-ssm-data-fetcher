...
class MultiCSVGenerator(BaseOutputGenerator):
    ...
    def generate(self, data: List[Dict]) -> str:
        ...
        try:
            ...
            base_filename = base_filename.replace('.csv', '')  # Remove extension if present
            ...
            for sheet_name, sheet_df in sheets_data.items():
                ...
                self.logger.info(f"  - Generated: {csv_filename} ({len(sheet_df)} rows)")
            
            # Log summary
            stats = self._get_data_statistics(data)
            self.logger.info(f"Multi-CSV generation completed: {len(generated_files)} files")
    ...
    def _create_service_summary(self, data: List[Dict]) -> pd.DataFrame:
        ...
        return pd.DataFrame(summary_data).sort_values('Coverage Percentage', ascending=False)
    ...
    def _create_statistics(self, data: List[Dict]) -> pd.DataFrame:
        ...
        stats_data = [
            ...
            {'Metric': 'Average Services per Region', 'Value': round(avg_services_per_region, 2)},
            {'Metric': 'Average Regions per Service', 'Value': round(avg_regions_per_service, 2)},

...
class ServiceMapper(BaseProcessor):
        ...
        for service_code in input_data:
            if not isinstance(service_code, str):
                raise ProcessingValidationError(
                    f"Service code must be string: {service_code}"
                )
            ...
            if not all(c.isalnum() or c in "-_" for c in service_code):
                raise ProcessingValidationError(
                    f"Invalid service code format: {service_code}"
                )
    ...
    def process(self, input_data: List[str], **kwargs) -> Dict[str, List[str]]:
        ...
        self.logger.info(
            f"Mapping {len(input_data)} services to regions using AWS SSM data"
        )
        ...
        try:
            for i, service_code in enumerate(input_data, 1):
                self.logger.info(
                    f"Processing service {i:3d}/{len(input_data)}: {service_code}"
                )

                try:
                    # Fetch regions for this service with reliability patterns
                    service_regions = self._fetch_service_regions_with_reliability(
                        service_code
                    )
                    ...
                    self.logger.warning(
                        f"Failed to process service {service_code}: {e}"
                    )
                    continue
    ...
    def get_region_services(
        self, region_code: str, all_services: List[str]
    ) -> List[str]:
    ...
    def get_coverage_stats(self, services: List[str]) -> Dict[str, Any]:
        ...
        for service in services:
            regions_with_service = sum(
                1
                for region_services in region_services_map.values()
                if service in region_services
            )
            service_coverage[service] = {
                "region_count": regions_with_service,
                "coverage_percentage": (
                    (regions_with_service / total_regions * 100)
                    if total_regions > 0
                    else 0
                ),
            }
        ...
        for region, region_services in region_services_map.items():
            coverage_pct = (
                (len(region_services) / total_services * 100)
                if total_services > 0
                else 0
            )
        ...
        avg_services_per_region = (
            total_mappings / total_regions if total_regions > 0 else 0
        )
        avg_regions_per_service = (
            total_mappings / total_services if total_services > 0 else 0
        )
...
class RegionalServiceMapper(ServiceMapper):
    ...
    def analyze_regional_distribution(self, services: List[str]) -> Dict[str, Any]:
        ...
        region_sizes = [
            (region, len(svc_list)) for region, svc_list in region_services_map.items()
        ]
        ...
        service_by_availability = sorted(
            service_availability.items(), key=lambda x: x[1]["count"], reverse=True
        )
        ...
        return {
            ...
            "distribution_metrics": {
                "regions_with_all_services": sum(
                    1
                    for region, svc_list in region_services_map.items()
                    if len(svc_list) == len(services)
                ),
                "services_in_all_regions": sum(
                    1
                    for service in services
                    if service_availability[service]["count"]
                    == len(region_services_map)
                ),
                "coverage_variance": self._calculate_coverage_variance(
                    region_services_map, services
                ),
            },
        }

    def _calculate_coverage_variance(
        self, region_services_map: Dict[str, List[str]], services: List[str]
    ) -> float:
        ...
        variance = sum((x - mean_coverage) ** 2 for x in coverage_percentages) / len(
            coverage_percentages
        )

...
from ..core.error_handling import with_retry, ErrorHandler
...
class AvailabilityZoneAnalyzer(BaseProcessor):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        for region_code in input_data:
            if not isinstance(region_code, str):
                raise ProcessingValidationError(f"Region code must be string: {region_code}")
            ...
            if not re.match(r'^[a-z0-9-]+$', region_code):
                raise ProcessingValidationError(f"Invalid region code format: {region_code}")
        ...
        try:
            ...
            for region in input_data:
                ...
                if az_count > 0:
                    ...
                    self.logger.debug(f"Using fallback count {fallback_count} AZs for {region}")
            
            self.logger.info(f"Successfully analyzed AZ data for {len(az_data)} regions")
            return az_data
            
        except Exception as e:
            self.logger.error(f"AZ analysis failed: {e}", exc_info=True)
            raise StatisticsAnalysisError(f"Failed to analyze availability zones: {e}") from e
    ...
    def _fetch_all_ssm_parameters_by_path(self, parameter_path: str) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to fetch parameters for path {parameter_path}: {e}")
...
class StatisticsAnalyzer(BaseProcessor):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        """
        if not isinstance(input_data, list):
            raise ProcessingValidationError("Input must be a list of service-region mappings")
        ...
        for i, item in enumerate(input_data[:sample_size]):
            ...
            if missing_fields:
                raise ProcessingValidationError(f"Item {i} missing required fields: {missing_fields}")
        
        return True
    
    def process(self, input_data: List[Dict], analysis_type: str = "comprehensive", **kwargs) -> Dict[str, Any]:
        ...
        self.logger.info(f"Performing {analysis_type} statistical analysis on {len(input_data)} records")
        ...
        try:
            ...
            raise StatisticsAnalysisError(f"Failed to perform {analysis_type} analysis: {e}") from e
    ...
    def comprehensive_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        total_services = df['Service Code'].nunique() if 'Service Code' in df.columns else df['Service Name'].nunique()
        ...
        coverage_stats = {
            ...
            'overall_coverage_percentage': round((total_mappings / (total_regions * total_services)) * 100, 2),
            'average_service_availability': round((total_mappings / total_services) / total_regions * 100, 2),
            'average_regional_coverage': round((total_mappings / total_regions) / total_services * 100, 2)
        }
    ...
    def regional_distribution_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        # Geographic grouping
        geographic_groups = self._group_regions_geographically(region_counts.index.tolist())
    ...
    def service_coverage_analysis(self, data: List[Dict], all_services: List[str] = None, **kwargs) -> Dict[str, Any]:
        ...
        high_coverage = service_coverage[(service_coverage >= total_regions * 0.8) & 
                                        (service_coverage < total_regions)]
        medium_coverage = service_coverage[(service_coverage >= total_regions * 0.5) & 
        ...
        if all_services:
            ...
            missing_services_analysis = {
                ...
                'missing_percentage': round((len(missing_services) / len(all_services)) * 100, 2),
        ...
        return {
            ...
            'service_categories': self._categorize_services_by_coverage(service_coverage, total_regions)
        }
    ...
    def availability_zone_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        if len(correlation_df) >= 3:
            correlation = correlation_df['az_count'].corr(correlation_df['service_count'])
        ...
        return {
            'az_summary': {
                ...
                'avg_azs_per_region': round(sum(az_data.values()) / len(az_data), 2) if az_data else 0,
            ...
            'az_service_correlation': {
                ...
                'correlation_interpretation': self._interpret_correlation(correlation) if correlation is not None else None
            },
    ...
    def geographic_distribution_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        for geo_region, regions in region_groups.items():
            ...
            if not region_data.empty:
                geographic_analysis[geo_region] = {
                    ...
                    'total_services': region_data['Service Code'].nunique() if 'Service Code' in region_data.columns else region_data['Service Name'].nunique(),
                    'total_mappings': len(region_data),
                    'avg_services_per_region': round(len(region_data) / len(regions), 2),
                    'unique_services': len(set(region_data['Service Code'] if 'Service Code' in region_data.columns else region_data['Service Name'])),
        ...
        return {
            ...
            'geographic_coverage_leaders': self._find_geographic_leaders(geographic_analysis),
            'geographic_service_gaps': self._identify_geographic_gaps(service_geographic_presence, region_groups)
        }
    ...
    def service_pattern_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        for service in df[service_col].unique():
            ...
            if any(keyword in service_lower for keyword in ['ec2', 'lambda', 'batch', 'compute', 'ecs', 'eks', 'fargate']):
                patterns['compute_services'].append(service)
            elif any(keyword in service_lower for keyword in ['s3', 'ebs', 'efs', 'fsx', 'storage', 'glacier']):
                patterns['storage_services'].append(service)
            elif any(keyword in service_lower for keyword in ['rds', 'dynamodb', 'aurora', 'database', 'db', 'dax']):
                patterns['database_services'].append(service)
            elif any(keyword in service_lower for keyword in ['vpc', 'elb', 'cloudfront', 'route53', 'network', 'api']):
                patterns['networking_services'].append(service)
            elif any(keyword in service_lower for keyword in ['iam', 'kms', 'security', 'waf', 'shield', 'cognito']):
                patterns['security_services'].append(service)
            elif any(keyword in service_lower for keyword in ['athena', 'glue', 'kinesis', 'analytics', 'redshift', 'emr']):
                patterns['analytics_services'].append(service)
            elif any(keyword in service_lower for keyword in ['sagemaker', 'comprehend', 'rekognition', 'ml', 'ai', 'lex']):
                patterns['ml_ai_services'].append(service)
            elif any(keyword in service_lower for keyword in ['code', 'developer', 'build', 'deploy', 'pipeline']):
                patterns['developer_tools'].append(service)
            elif any(keyword in service_lower for keyword in ['cloudwatch', 'cloudtrail', 'config', 'systems', 'ssm']):
        ...
        for pattern_name, services in patterns.items():
            if services:
                ...
                pattern_coverage[pattern_name] = {
                    ...
                    'avg_regional_availability': round(len(pattern_data) / len(services), 2) if services else 0,
    ...
    def performance_metrics_analysis(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        data_efficiency = {
            'records_per_operation': len(data) / max(processing_stats['total_operations'], 1),
            'cache_efficiency': processing_stats.get('cache_hit_rate', 0) * 100,
            'processing_speed': len(data) / max(processing_stats['total_processing_time'], 0.001),
        ...
        return {
            ...
            'recommendations': self._generate_performance_recommendations(data_efficiency, data_quality)
        }
    ...
    def _categorize_regions_by_service_count(self, region_counts: pd.Series) -> Dict[str, List[str]]:
        ...
        return {
            ...
            'tier_2_medium_high': region_counts[(region_counts >= q50) & (region_counts < q75)].index.tolist(),
            'tier_3_medium_low': region_counts[(region_counts >= q25) & (region_counts < q50)].index.tolist(),
    ...
    def _identify_regional_gaps(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            'underserved_regions': region_counts[region_counts < (mean_count - std_count)].to_dict(),
            'overserved_regions': region_counts[region_counts > (mean_count + std_count)].to_dict(),
    ...
    def _calculate_regional_diversity(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        for region in df['Region Code'].unique():
            ...
            if total_services > 0:
                ...
                shannon_diversity = -sum((count / total_services) * 
                                       math.log(count / total_services) 
                                       for count in service_counts.values() if count > 0)
    ...
    def _identify_service_patterns(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i, service1 in enumerate(services):
            for service2 in services[i+1:]:
                overlap = len(service_regions[service1].intersection(service_regions[service2]))
                total_regions = len(service_regions[service1].union(service_regions[service2]))
                
                if total_regions > 0:
                    jaccard_similarity = overlap / total_regions
                    if jaccard_similarity > 0.5:  # Services appear together in >50% of regions
                        co_occurrence[f"{service1}+{service2}"] = round(jaccard_similarity, 3)
        
        return {
            'high_co_occurrence_pairs': dict(sorted(co_occurrence.items(), 
                                                  key=lambda x: x[1], 
                                                  reverse=True)[:10])
        }
    
    def _categorize_services_by_coverage(self, service_coverage: pd.Series, total_regions: int) -> Dict[str, List[str]]:
        """Categorize services by their regional coverage."""
        return {
            'universal_coverage': service_coverage[service_coverage == total_regions].index.tolist(),
            'high_coverage': service_coverage[service_coverage >= total_regions * 0.8].index.tolist(),
            'medium_coverage': service_coverage[(service_coverage >= total_regions * 0.5) & 
                                              (service_coverage < total_regions * 0.8)].index.tolist(),
            'low_coverage': service_coverage[service_coverage < total_regions * 0.5].index.tolist()
        }
    ...
    def _find_geographic_leaders(self, geographic_analysis: Dict[str, Any]) -> Dict[str, str]:
        ...
        for geo_region, stats in geographic_analysis.items():
            if stats['regions']:
                ...
                leaders[geo_region] = max(stats['regions'], key=lambda x: stats['avg_services_per_region'])
        return leaders
    
    def _identify_geographic_gaps(self, service_presence: Dict[str, Dict], region_groups: Dict[str, List]) -> Dict[str, List[str]]:
    ...
    def _calculate_pattern_statistics(self, pattern_coverage: Dict[str, Dict]) -> Dict[str, Any]:
        """Calculate statistics across service patterns."""
        if not pattern_coverage:
            return {}
        
        avg_availability = [stats['avg_regional_availability'] for stats in pattern_coverage.values()]
        service_counts = [stats['service_count'] for stats in pattern_coverage.values()]
        
        return {
            'most_available_pattern': max(pattern_coverage.items(), key=lambda x: x[1]['avg_regional_availability'])[0],
            'largest_pattern': max(pattern_coverage.items(), key=lambda x: x[1]['service_count'])[0],
            'pattern_availability_variance': round(stats.stdev(avg_availability) if len(avg_availability) > 1 else 0, 3),
    ...
    def _calculate_data_completeness(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            'overall_completeness': round((1 - df.isnull().sum().sum() / df.size) * 100, 2),
            'field_completeness': {col: round((1 - df[col].isnull().sum() / len(df)) * 100, 2) 
    ...
    def _calculate_data_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'unique_services': df['Service Code'].nunique() if 'Service Code' in df.columns else df['Service Name'].nunique(),
    ...
    def _calculate_coverage_metrics(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        return {
            'regional_coverage': round((total_regions / 40) * 100, 2),  # Assume 40 total AWS regions
            'service_coverage': round((total_services / 400) * 100, 2),  # Assume 400 total AWS services
            'mapping_density': round((len(df) / (total_regions * total_services)) * 100, 2)
        }
    
    def _check_format_consistency(self, df: pd.DataFrame) -> Dict[str, bool]:
        """Check format consistency of key fields."""
        return {
            'region_code_format': all(re.match(r'^[a-z0-9-]+$', str(code)) for code in df['Region Code'].dropna()),
            'service_code_format': all(re.match(r'^[a-z0-9-]+$', str(code)) for code in df.get('Service Code', pd.Series()).dropna()),
        }
    
    def _generate_performance_recommendations(self, efficiency: Dict, quality: Dict) -> List[str]:
        """Generate performance improvement recommendations."""
        recommendations = []
        
        if efficiency['cache_efficiency'] < 80:
            recommendations.append("Consider increasing cache retention to improve efficiency")
        ...
        if quality['completeness']['overall_completeness'] < 95:
            recommendations.append("Data completeness below 95% - review data collection processes")

...
class DataTransformer(BaseProcessor):
    ...
    def process(self, input_data: List[Dict], transformation_type: str = "service_matrix", **kwargs) -> Any:
        ...
        self.logger.info(f"Applying {transformation_type} transformation to {len(input_data)} records")
        ...
        try:
            ...
            self.logger.info(f"Successfully applied {transformation_type} transformation")
            return result
            
        except Exception as e:
            self.logger.error(f"Transformation {transformation_type} failed: {e}", exc_info=True)
            raise DataTransformationError(f"Failed to apply {transformation_type} transformation: {e}") from e
    ...
    def generate_service_matrix(self, data: List[Dict], **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(f"Creating service matrix: {len(services)} services × {len(regions)} regions")
    ...
    def generate_region_summary(self, data: List[Dict], region_names: Dict[str, str] = None,
                               rss_data: Dict[str, Dict] = None, az_data: Dict[str, int] = None,
                               all_services: List[str] = None, **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(f"Generating region summary for {df['Region Code'].nunique()} regions")
    ...
    def generate_service_summary(self, data: List[Dict], all_services: List[str] = None,
                                service_names: Dict[str, str] = None, **kwargs) -> pd.DataFrame:
        ...
        if all_services:
            self.logger.info(f"Analyzing {len(all_services)} total services for regional availability")
            ...
            for service_code in sorted(all_services):
                ...
                summary_data.append({
                    ...
                    'Coverage %': coverage_pct / 100  # Store as decimal for Excel formatting
                })
        else:
    ...
    def generate_statistics(self, data: List[Dict], all_services: List[str] = None, **kwargs) -> pd.DataFrame:
    ...
    def generate_pivot_table(self, data: List[Dict], rows: str = 'Region Code',
                            columns: str = 'Service Code', values: str = 'Service Name',
                            aggfunc: str = 'count', **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(f"Creating pivot table: rows={rows}, columns={columns}, values={values}")
    ...
    def generate_coverage_analysis(self, data: List[Dict], all_services: List[str] = None,
                                  **kwargs) -> Dict[str, Any]:
        ...
        # Overall statistics
        coverage_values = [info['coverage_percentage'] for info in region_coverage.values()]
        availability_values = [info['availability_percentage'] for info in service_availability.values()]
        ...
        analysis = {
            'overview': {
                ...
                'avg_regions_per_service': round(len(df) / df['Service Name'].nunique(), 1)
            },
            'regional_coverage': {
                'by_region': region_coverage,
                'statistics': {
                    'mean_coverage': round(sum(coverage_values) / len(coverage_values), 2),
            ...
            'service_availability': {
                ...
                'statistics': {
                    'mean_availability': round(sum(availability_values) / len(availability_values), 2),
    ...
    def transform_to_hierarchical(self, data: List[Dict], group_by: str = 'Region Code') -> Dict[str, List[Dict]]:

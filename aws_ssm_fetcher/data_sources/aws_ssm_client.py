...
class AWSSSMClient(AWSDataSource):
    ...
    def __init__(self, aws_session=None, cache_manager=None, region="us-east-1", max_retries=3, base_delay=1.0):
    ...
    def discover_regions(self) -> List[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to discover regions from SSM ({error_category}): {e}")
    ...
    def discover_services(self) -> List[str]:
        ...
        try:
            ...
            services.sort()
            self.logger.info(f"Successfully discovered {len(services)} services from SSM")
            ...
            # Classify error for appropriate handling
            should_retry, error_category = self._error_handler.classify_aws_error(e)
            self.logger.error(f"Failed to discover services from SSM ({error_category}): {e}")
            
            if should_retry:
                raise RetryableError(f"Retryable error discovering services: {e}") from e
            else:
                # Non-retryable errors return empty list rather than raising
                self.logger.warning("Non-retryable error, returning empty list")
                return []
    ...
    def get_service_regions(self, service_code: str) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to get regions for service {service_code}: {e}")
            return []
    ...
    def fetch_service_names(self, services: List[str]) -> Dict[str, str]:
        ...
        for service_code in services:
            try:
                ...
                self.logger.warning(f"Failed to get name for service {service_code}: {e}")
                service_names[service_code] = service_code  # Fallback to code
            
            # Small delay to avoid throttling
            time.sleep(0.05)
        
        self.logger.info(f"Successfully fetched names for {len(service_names)} services")
    ...
    def get_parameters_batch(self, parameter_paths: List[str]) -> Dict[str, Optional[str]]:
    ...
    def fetch_all_ssm_parameters_by_path(self, parameter_path: str) -> List[str]:
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                self.logger.info(f"Found {total_params} total parameters at path {parameter_path}")
                ...
                if 'ThrottlingException' in str(e) and retry_count < self.max_retries:
                    ...
                    self.logger.warning(f"Throttling detected, retrying in {delay}s (attempt {retry_count}/{self.max_retries})")
                    time.sleep(delay)
                else:
                    self.logger.error(f"Failed to fetch parameters at path {parameter_path}: {e}")
                    break
            except Exception as e:
                self.logger.error(f"Failed to fetch parameters at path {parameter_path}: {e}")
                break
    ...
    def discover_regions_from_ssm_enhanced(self) -> List[str]:
        ...
        try:
            ...
            regions_params = self.fetch_all_ssm_parameters_by_path('/aws/service/global-infrastructure/regions')
            ...
            self.logger.info(f"Found {len(discovered_regions)} regions from direct path")
            
            # If we didn't find many regions, try targeted parameter sampling
            if len(discovered_regions) < 10:
                self.logger.info("Limited regions found, trying targeted parameter sampling...")
                
                # Sample from service parameters to find more regions
                service_params = self.fetch_all_ssm_parameters_by_path('/aws/service/global-infrastructure/services')
                
                for param_name in service_params[:50]:  # Sample first 50 service parameters
                    # Look for regional service availability patterns
                    region_match = re.search(r'/services/[^/]+/regions/([^/]+)', param_name)
                    if region_match:
                        region_code = region_match.group(1)
                        if region_code and region_code not in discovered_regions:
                            discovered_regions.append(region_code)
        ...
        try:
            ...
            for param_name in service_params:
                ...
                if pages_processed % 100 == 0:
                    self.logger.info(f"Processed {pages_processed} pages, found {len(services)} unique services so far...")
            ...
            if len(discovered_services) < 200:
                self.logger.info(f"Found {len(services)} services with non-recursive scan, trying recursive approach to find more...")
    ...
    def get_region_service_mapping(self, regions: List[str], services: List[str]) -> Dict[str, List[str]]:
        ...
        try:
            for i, service_code in enumerate(services):
                self.logger.info(f"Processing service {i+1:3d}/{len(services)}: {service_code}")
                ...
                try:
                    ...
                    for region in service_regions:
                        if region in regions:  # Only include regions we're interested in
            ...
            self.logger.info(f"Successfully mapped {len(region_services)} regions with {total_mappings} total service mappings")

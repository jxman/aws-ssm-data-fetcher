...
class DataSourceManager:
    ...
    def __init__(self, config=None, cache_manager=None, aws_session=None):
        ...
        self.fallback_services = [
            'ec2', 's3', 'rds', 'lambda', 'dynamodb', 'cloudformation', 'iam', 'cloudwatch',
            'sns', 'sqs', 'elasticloadbalancing', 'autoscaling', 'ecs', 'eks', 'vpc',
            'route53', 'cloudfront', 'apigateway', 'secretsmanager', 'ssm'
        ]
    ...
    def fetch_regions(self, strategy: DataSourceStrategy = DataSourceStrategy.AUTO) -> List[str]:
        ...
        try:
            if strategy == DataSourceStrategy.SSM_ONLY:
                ...
                if not regions or len(regions) < 5:  # Minimum reasonable number
                    self.logger.warning("SSM returned insufficient regions, trying RSS fallback")
                ...
                if not regions or len(regions) < 5:
                    self.logger.warning("RSS returned insufficient regions, trying SSM fallback")
    ...
    def fetch_services(self, strategy: DataSourceStrategy = DataSourceStrategy.AUTO) -> List[str]:
        ...
        try:
            if strategy in [DataSourceStrategy.SSM_ONLY, DataSourceStrategy.SSM_WITH_RSS_FALLBACK]:
                services = self._fetch_services_from_ssm()
                if services and len(services) >= 50:  # Reasonable minimum
                    return services
                else:
                    self.logger.warning("SSM returned insufficient services, using fallback")
    ...
    def fetch_region_metadata(self, regions: List[str]) -> Tuple[Dict[str, str], Dict[str, Dict]]:
        ...
        try:
            ...
            for service in services:
                if service not in service_names or not service_names[service]:
                    service_names[service] = self._generate_service_display_name(service)
            ...
            return {service: self._generate_service_display_name(service) for service in services}
    
    def fetch_region_service_mapping(self, regions: List[str], services: List[str]) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if not mapping:
                self.logger.warning("No region-service mapping returned, generating fallback")
    ...
    def validate_data_consistency(self, regions: List[str], services: List[str], 
                                 region_service_mapping: Dict[str, List[str]]) -> bool:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._performance_stats,
            'cache_hit_rate': self._performance_stats['cache_hits'] / max(self._performance_stats['total_requests'], 1),
            'fallback_rate': self._performance_stats['fallback_used'] / max(self._performance_stats['total_requests'], 1)
        }
    ...
    def _get_fallback_regions(self) -> List[str]:
        ...
        self.logger.info(f"Using fallback regions ({len(self.fallback_regions)} regions)")
    ...
    def _get_fallback_services(self) -> List[str]:
        ...
        self._performance_stats['fallback_used'] += 1
        self.logger.info(f"Using fallback services ({len(self.fallback_services)} services)")
    ...
    def _generate_fallback_mapping(self, regions: List[str], services: List[str]) -> Dict[str, List[str]]:
        ...
        for region in regions:
            ...
            available_services.extend(services[:min(len(services), 20)])  # Add first 20 services
    ...
    def _is_valid_service_code(self, service_code: str) -> bool:
        ...
        return bool(service_code and isinstance(service_code, str) and len(service_code) > 1)
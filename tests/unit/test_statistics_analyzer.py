...
from aws_ssm_fetcher.processors.statistics_analyzer import StatisticsAnalyzer, AvailabilityZoneAnalyzer
...
def create_comprehensive_test_data():
    ...
    return [
        ...
        {'Region Code': 'us-east-1', 'Service Code': 'sagemaker', 'Service Name': 'Amazon SageMaker'},
...
def test_statistics_analyzer():
    ...
    context = ProcessingContext(
        config=config,
        cache_manager=cache_manager,
        logger_name="test_stats_analyzer"
    )
    context.ssm_client = mock_ssm_client
    ...
    try:
        ...
        print(f"   Mean services per region: {regional_stats['mean_services_per_region']}")
        print(f"   Max services per region: {regional_stats['max_services_per_region']}")
    ...
    try:
        all_services = ['ec2', 's3', 'lambda', 'rds', 'dynamodb', 'sagemaker', 'ecs', 'eks']
    ...
    try:
        ...
        regions = ['us-east-1', 'us-west-2', 'eu-west-1', 'ap-south-1', 'af-south-1', 'ca-central-1']
        az_cache_key = stats_analyzer.az_analyzer.get_cache_key(regions)
        az_test_data = {
            'us-east-1': 6, 'us-west-2': 4, 'eu-west-1': 3,
            'ap-south-1': 3, 'af-south-1': 3, 'ca-central-1': 3
        }
        ...
        print(f"   Total regions with AZ data: {az_summary['total_regions_with_az_data']}")
        ...
        if correlation['correlation_coefficient']:
            print(f"   AZ-Service correlation: {correlation['correlation_coefficient']} ({correlation['correlation_interpretation']})")
    ...
    try:
        ...
        for geo_region, stats in geo_regions.items():
            print(f"   {geo_region}: {stats['region_count']} regions, {stats['total_services']} unique services")
    ...
    try:
        pattern_result = stats_analyzer.process(
            test_data,
            analysis_type="service_patterns"
        )
        ...
        for pattern_name, stats in pattern_coverage.items():
            if stats['service_count'] > 0:
                print(f"   {pattern_name}: {stats['service_count']} services, {stats['avg_regional_availability']:.1f} avg regions")
...
def test_error_handling():
    ...
    try:
        stats_analyzer.process(create_comprehensive_test_data(), analysis_type="invalid_analysis")

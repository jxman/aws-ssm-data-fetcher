...
class SSMLogger:
    ...
    def _log_with_extra(self, level: int, message: str, extra: Dict[str, Any], exc_info: bool = False):
    ...
    def timer(self, operation: str):
        ...
        try:
            ...
            duration = time.time() - start_time
            self.error(f"Failed {operation}", duration_seconds=f"{duration:.2f}", 
    ...
    def end_timer(self, operation: str, **extra):
        ...
        self.info(f"Completed {operation}", 
                 duration_seconds=f"{duration:.2f}", **extra)
        return duration

            ...
            for attempt in range(retry_config.max_attempts):
                try:
                    ...
                    if attempt == retry_config.max_attempts - 1:
                        logger.error(f"All {retry_config.max_attempts} attempts failed. Last error: {e}")
                        raise e
                    ...
                    if any(isinstance(e, exc_type) for exc_type in retry_config.retryable_exceptions):
                        if attempt == retry_config.max_attempts - 1:
                            logger.error(f"All {retry_config.max_attempts} attempts failed. Last error: {e}")
                            raise e
...
class ErrorHandler:
    ...
    def classify_aws_error(self, error: Exception) -> Tuple[bool, str]:
        ...
        if any(keyword in error_str for keyword in ['throttling', 'throttled', 'rate exceeded']):
        ...
        if any(keyword in error_str for keyword in ['service unavailable', '503', '502', '504']):
            return True, 'service_unavailable'
        
        # Authentication errors - don't retry
        if any(keyword in error_str for keyword in ['access denied', 'unauthorized', '401', '403']):
        ...
        if any(keyword in error_str for keyword in ['validation', 'invalid parameter', 'bad request', '400']):

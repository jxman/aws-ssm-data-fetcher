...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        from aws_ssm_fetcher.processors.pipeline import ProcessingPipeline, PipelineOrchestrator
        ...
        lambda_config.aws_region = raw_data.get('config', {}).get('aws_region', 'us-east-1')
        ...
        pipeline_results = orchestrator.execute_pipeline(pipeline_config, pipeline_input)
        ...
        if 'discovery' in pipeline_results.get('stage_results', {}):
            ...
            # Store metadata
            processed_data['metadata']['regions'] = {r.get('code', r): r.get('name', r) for r in regions}
            processed_data['metadata']['services'] = {s.get('code', s): s.get('name', s) for s in services}
        ...
        if 'mapping' in pipeline_results.get('stage_results', {}):
            ...
            for service, regions in service_mappings.items():
                service_name = processed_data['metadata']['services'].get(service, service)
                for region in regions:
                    region_name = processed_data['metadata']['regions'].get(region, region)

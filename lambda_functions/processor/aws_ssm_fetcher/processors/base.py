...
class BaseProcessor(ABC):
    ...
    def __init__(self, context: ProcessingContext):
        ...
        self.logger = get_logger(f"{context.logger_name}.{self.__class__.__name__.lower()}")
    ...
    def process_with_cache(self, input_data: Any, **kwargs) -> Any:
        ...
        try:
            ...
            # Process fresh data
            self.logger.info(f"Processing {len(input_data) if hasattr(input_data, '__len__') else 'data'} items...")
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self._processing_stats,
            'success_rate': self._processing_stats['successful_operations'] / max(total_ops, 1),
            'failure_rate': self._processing_stats['failed_operations'] / max(total_ops, 1),
            'average_processing_time': self._processing_stats['total_processing_time'] / max(total_ops, 1),
            'cache_hit_rate': self._processing_stats['cache_hits'] / max(
                self._processing_stats['cache_hits'] + self._processing_stats['cache_misses'], 1
            )
        }

...
class RegionDiscoverer(BaseProcessor):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        if input_data is not None:
            if not isinstance(input_data, dict):
                raise ProcessingValidationError("Input must be a dictionary of discovery parameters")
            
            # Validate discovery parameters
            valid_params = ['max_pages', 'recursive', 'region_pattern', 'validate_regions']
    ...
    def process(self, input_data: Optional[Dict] = None, **kwargs) -> List[str]:
        ...
        try:
            ...
            if validate_regions:
                validated_regions = self._validate_discovered_regions(discovered_regions)
                self.logger.info(f"Validated {len(validated_regions)}/{len(discovered_regions)} discovered regions")
                discovered_regions = validated_regions
            
            self.logger.info(f"Successfully discovered {len(discovered_regions)} regions")
            ...
            self.context.metadata.update({
                'region_discovery_stats': {
                    ...
                    'discovery_method': 'directory + recursive' if use_recursive else 'directory_only',
    ...
    def _get_parameters_by_path(self, path: str, recursive: bool = False, max_results: int = 10) -> Dict:
    ...
    def _validate_discovered_regions(self, regions: List[str]) -> List[str]:
        ...
        for region in regions:
            ...
            is_valid = any(re.match(pattern, region) for pattern in self.region_patterns)
...
class ServiceDiscoverer(BaseProcessor):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        if input_data is not None:
            ...
            valid_params = ['max_pages', 'use_recursive', 'validate_services', 'min_expected_services']
            for param in input_data:
                if param not in valid_params:
                    raise ProcessingValidationError(f"Unknown parameter: {param}")
    ...
    def process(self, input_data: Optional[Dict] = None, **kwargs) -> List[str]:
        ...
        try:
            ...
            if len(services) < min_expected_services and use_recursive:
                self.logger.info(f"Found {len(services)} services, trying recursive discovery for more...")
            ...
            if validate_services:
                validated_services = self._validate_discovered_services(discovered_services)
                self.logger.info(f"Validated {len(validated_services)}/{len(discovered_services)} discovered services")
                discovered_services = validated_services
            
            self.logger.info(f"Successfully discovered {len(discovered_services)} services")
            ...
            self.context.metadata.update({
                'service_discovery_stats': {
                    ...
                    'discovery_method': 'directory + recursive' if use_recursive else 'directory_only',
                    'discovery_timestamp': datetime.now().isoformat(),
                    'service_categories': self._categorize_services(discovered_services)
                }
            })
        ...
        try:
            ...
            for page in page_iterator:
                ...
                if pages_processed % 25 == 0:
                    self.logger.debug(f"Directory discovery: processed {pages_processed} pages, found {len(services)} services")
        ...
        try:
            ...
            for page in page_iterator:
                ...
                if pages_processed % 50 == 0:
                    self.logger.debug(f"Fetched {len(parameter_names)} parameters from {pages_processed} pages")
    ...
    def _validate_discovered_services(self, services: List[str]) -> List[str]:
        ...
        for service in services:
            # Check basic format
            is_valid_format = any(re.match(pattern, service) for pattern in self.service_patterns)
            
            # Additional validation: reasonable length and characters
            if (is_valid_format and 
                2 <= len(service) <= 50 and 
                service.replace('-', '').replace('_', '').isalnum()):
    ...
    def _categorize_services(self, services: List[str]) -> Dict[str, int]:
        ...
        for service in services:
            ...
            for category, prefixes in self.known_service_prefixes.items():
                if any(service_lower.startswith(prefix) or prefix in service_lower for prefix in prefixes):
...
class RegionalDataValidator(BaseProcessor):
    ...
    def validate_input(self, input_data: Any) -> bool:
        ...
        if not isinstance(input_data, list):
            raise ProcessingValidationError("Input must be a list of regional data mappings")
    ...
    def process(self, input_data: List[Dict], validation_type: str = "comprehensive", **kwargs) -> Dict[str, Any]:
        ...
        self.logger.info(f"Performing {validation_type} validation on {len(input_data)} regional mappings")
        ...
        try:
            ...
            self.logger.error(f"Validation {validation_type} failed: {e}", exc_info=True)
            raise ValidationError(f"Failed to perform {validation_type} validation: {e}") from e
    ...
    def comprehensive_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            results['anomaly_detection'] = self.anomaly_detection_validation(data, **kwargs)
            ...
            results['summary'] = {
                ...
                'total_validations_performed': len(results) - 1,  # Exclude summary itself
    ...
    def data_integrity_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        actual_issues = sum([stats['duplicate_records'], stats['missing_fields'], 
                           stats['invalid_formats'], stats['empty_values']])
        integrity_score = max(0, (total_possible_issues - actual_issues) / total_possible_issues * 100)
    ...
    def coverage_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        actual_coverage = (total_mappings / expected_total_mappings) * 100 if expected_total_mappings > 0 else 0
        ...
        if unique_regions < self.validation_thresholds['min_regions']:
            coverage_issues.append(f"Too few regions: {unique_regions} < {self.validation_thresholds['min_regions']}")
        
        if unique_services < self.validation_thresholds['min_services']:
            coverage_issues.append(f"Too few services: {unique_services} < {self.validation_thresholds['min_services']}")
        
        if actual_coverage < self.validation_thresholds['expected_coverage_percentage']:
            coverage_issues.append(f"Low coverage: {actual_coverage:.1f}% < {self.validation_thresholds['expected_coverage_percentage']}%")
        
        # Calculate coverage score
        coverage_score = min(100, (actual_coverage / self.validation_thresholds['expected_coverage_percentage']) * 100)
    ...
    def consistency_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        region_consistency = {
            ...
            'coefficient_of_variation': region_service_counts.std() / region_service_counts.mean() if region_service_counts.mean() > 0 else 0,
            'outlier_regions': []
        }
        ...
        for region, count in region_service_counts.items():
            if abs(count - mean_services) > 2 * std_services:
                ...
                consistency_issues.append(f"Region {region} has unusual service count: {count}")
        
        # Service consistency checks
        service_region_counts = df.groupby('Service Code').size()
        service_consistency = {
            'std_dev': service_region_counts.std(),
            'coefficient_of_variation': service_region_counts.std() / service_region_counts.mean() if service_region_counts.mean() > 0 else 0,
            'outlier_services': []
        }
        ...
        for service, count in service_region_counts.items():
            if abs(count - mean_regions) > 2 * std_regions:
                ...
                consistency_issues.append(f"Service {service} has unusual region count: {count}")
    ...
    def discovery_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            total_found = len(data_regions.intersection(set(discovered_regions))) + len(data_services.intersection(set(discovered_services)))
            alignment_score = (total_found / total_discovered * 100) if total_discovered > 0 else 0
            
            discovery_issues = []
            if missing_regions:
                discovery_issues.append(f"Missing {len(missing_regions)} discovered regions")
            if extra_regions:
                discovery_issues.append(f"Found {len(extra_regions)} unknown regions")
            if missing_services:
                discovery_issues.append(f"Missing {len(missing_services)} discovered services")
            if extra_services:
    ...
    def anomaly_detection_validation(self, data: List[Dict], **kwargs) -> Dict[str, Any]:
        ...
        if region_std > 0:
            ...
            for region, z_score in region_anomalies.items():
                anomalies.append(f"Region {region} has anomalous service count (z-score: {z_score:.2f})")
        ...
        if service_std > 0:
            ...
            for service, z_score in service_anomalies.items():
                anomalies.append(f"Service {service} has anomalous region count (z-score: {z_score:.2f})")
        ...
        for service in expected_universal_services:
            if service in service_counts:
                ...
                if service_region_count < total_regions * 0.8:  # Should be in 80%+ of regions
                    anomalies.append(f"Universal service {service} only in {service_region_count}/{total_regions} regions")
                    anomaly_stats['pattern_anomalies'] += 1
        
        # Calculate anomaly score (lower is better, fewer anomalies)
        total_potential_anomalies = len(region_counts) + len(service_counts) + len(expected_universal_services)
        total_detected_anomalies = sum(anomaly_stats.values())
        anomaly_score = max(0, 100 - (total_detected_anomalies / total_potential_anomalies * 100))

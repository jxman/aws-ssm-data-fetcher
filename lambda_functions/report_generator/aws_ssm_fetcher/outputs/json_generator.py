...
class JSONGenerator(BaseOutputGenerator):
    ...
    def _create_json_structure(self, data: List[Dict]) -> Dict[str, Any]:
        ...
        if self.context.region_names or self.context.service_names or self.context.rss_data:
            ...
            if self.context.service_names:
                json_structure['enrichment']['service_names'] = self.context.service_names
    ...
    def _generate_analysis(self, data: List[Dict]) -> Dict[str, Any]:
        ...
        total_services = len(self.context.all_services) if self.context.all_services else len(service_coverage)
        ...
        analysis = {
            'service_coverage_by_region': {
                region: {
                    ...
                    'coverage_percentage': len(set(services)) / total_services if total_services > 0 else 0
                }
            ...
            'regional_coverage_by_service': {
                service: {
                    ...
                    'coverage_percentage': len(set(regions)) / len(region_coverage) if region_coverage else 0
                }
                for service, regions in service_coverage.items()
            },
            'top_regions_by_service_count': sorted(
                [(region, len(set(services))) for region, services in region_coverage.items()],
                key=lambda x: x[1],
                reverse=True
            )[:10],
            'top_services_by_region_count': sorted(
                [(service, len(set(regions))) for service, regions in service_coverage.items()],
                key=lambda x: x[1],
                reverse=True
            )[:10]
        }

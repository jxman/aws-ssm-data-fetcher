...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        from aws_ssm_fetcher.outputs.json_generator import JSONGenerator, CompactJSONGenerator
        from aws_ssm_fetcher.outputs.csv_generator import CSVGenerator, MultiCSVGenerator
            ...
            if format_name == 'multi_csv':
                ...
                for file_name in os.listdir(file_path):
                    if file_name.endswith('.csv'):
                        ...
                        s3_reports['csv_sheets'].append(f"s3://{s3_bucket}/{s3_report_key}")
            else:
                # Handle single files
        ...
        logger.info(f"All reports uploaded to S3 under: s3://{s3_bucket}/{reports_base_key}/")
        ...
        logger.info(f"Report generation completed: {json.dumps(response['statistics'])}")
        return response

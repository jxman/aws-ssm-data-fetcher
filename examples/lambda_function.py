...
class S3CacheManager:
    ...
    def __init__(self, bucket_name: str, cache_prefix: str = 'cache/', cache_hours: int = 24):
...
def fetch_region_rss_data(cache_manager: Optional[S3CacheManager] = None) -> Dict[str, Dict]:
...
def get_known_services():
    ...
    return [
        ...
        'appconfig', 'appconfigdata', 'appflow', 'application-autoscaling', 'appmesh',
        'apprunner', 'appstream', 'appsync', 'athena', 'autoscaling', 'backup', 'batch',
        'bedrock', 'budgets', 'chime', 'cloud9', 'cloudformation', 'cloudfront',
        'cloudsearch', 'cloudtrail', 'cloudwatch', 'codeartifact', 'codebuild',
        'codecommit', 'codedeploy', 'codeguru-reviewer', 'codepipeline', 'cognito-identity',
        'cognito-idp', 'comprehend', 'config', 'connect', 'databrew', 'dataexchange',
        'datapipeline', 'datasync', 'dax', 'detective', 'devicefarm', 'directconnect',
        'discovery', 'dms', 'docdb', 'ds', 'dynamodb', 'ebs', 'ec2', 'ecr', 'ecs',
        'efs', 'eks', 'elasticache', 'elasticbeanstalk', 'emr', 'events', 'firehose',
        'fms', 'forecast', 'frauddetector', 'fsx', 'gamelift', 'glacier',
        'globalaccelerator', 'glue', 'greengrass', 'groundstation', 'guardduty',
        'health', 'iam', 'imagebuilder', 'inspector', 'inspector2', 'iot',
        'iotanalytics', 'iotevents', 'kafka', 'kendra', 'kinesis', 'kinesisanalytics',
        'kms', 'lakeformation', 'lambda', 'license-manager', 'lightsail', 'logs',
        'machinelearning', 'managedblockchain', 'mediaconnect', 'mediaconvert',
        'medialive', 'mediapackage', 'mediastore', 'mediatailor', 'memorydb', 'mq',
        'mturk', 'neptune', 'networkmanager', 'organizations', 'outposts',
        'personalize', 'pinpoint', 'polly', 'pricing', 'qldb', 'quicksight', 'ram',
        'rds', 'redshift', 'rekognition', 'resource-groups', 'robomaker', 'route53',
        'route53domains', 'route53resolver', 's3', 's3control', 'sagemaker',
        'secretsmanager', 'securityhub', 'serverlessrepo', 'servicecatalog',
        'servicediscovery', 'ses', 'shield', 'signer', 'sns', 'sqs', 'ssm',
        'stepfunctions', 'storagegateway', 'sts', 'support', 'swf', 'synthetics',
        'textract', 'timestream', 'transcribe', 'transfer', 'translate', 'waf',
...
def fetch_region_names(ssm_client, regions: List[str], cache_manager: Optional[S3CacheManager] = None) -> Dict[str, str]:
...
def fetch_service_names(ssm_client, services: List[str], cache_manager: Optional[S3CacheManager] = None) -> Dict[str, str]:
...
def generate_regional_services(regions: List[str], services: List[str], service_names: Dict[str, str]) -> Dict[str, List[str]]:
    """Generate simplified regional service availability."""
    regional_services = {}
    
    # Core services available in most regions
    core_services = ['ec2', 's3', 'iam', 'cloudformation', 'cloudwatch', 'sns', 'sqs', 'lambda', 'dynamodb', 'rds']
    ...
    for region in regions:
        if region in ['us-east-1', 'us-west-2', 'eu-west-1', 'ap-southeast-1']:
            ...
            regional_services[region] = [s for s in core_services if s in valid_services]
        else:
            ...
            extras = ['autoscaling', 'cloudtrail', 'config', 'elasticache', 'logs', 'route53']
...
def generate_data_matrix(regions: List[str], region_names: Dict[str, str], 
...
def lambda_handler(event, context):
    ...
    try:
        ...
        cache_manager = None if disable_cache else S3CacheManager(
        ...
        data = generate_data_matrix(regions, region_names, service_names, regional_services)
